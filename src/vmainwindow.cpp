...
void VMainWindow::setupUI()
{
    ...
    m_mainSplitterFakeWidget = new QWidget(this);
    m_mainSplitterFakeWidget->hide();
...
QWidget *VMainWindow::setupDirectoryPanel()
{
    ...
    m_naviSplitterFakeWidget = new QWidget();
    ...
    m_naviSplitter->addWidget(m_naviSplitterFakeWidget);
    m_naviSplitter->setStretchFactor(0, 0);
    m_naviSplitter->setStretchFactor(1, 1);

    m_naviSplitterFakeWidget->hide();
...
void VMainWindow::enableCompactMode(bool p_enabled)
{
    ...
    if (p_enabled) {
        ...
        Q_ASSERT(m_mainSplitter->indexOf(m_mainSplitterFakeWidget) == -1);
        m_fileList->hide();
        QWidget *wid = m_mainSplitter->replaceWidget(fileListIdx, m_mainSplitterFakeWidget);
        m_mainSplitterFakeWidget->hide();

        // Insert m_fileList into m_naviSplitter.
        Q_ASSERT(m_naviSplitter->indexOf(m_naviSplitterFakeWidget) == fileListIdx);
        wid = m_naviSplitter->replaceWidget(fileListIdx, m_fileList);
        m_naviSplitterFakeWidget->setParent(this);
        m_naviSplitterFakeWidget->hide();
        ...
        Q_ASSERT(m_naviSplitter->indexOf(m_naviSplitterFakeWidget) == -1);
        m_fileList->hide();
        QWidget *wid = m_naviSplitter->replaceWidget(fileListIdx, m_naviSplitterFakeWidget);
        m_naviSplitterFakeWidget->hide();

        // Insert m_fileList into m_mainSplitter.
        Q_ASSERT(m_mainSplitter->indexOf(m_mainSplitterFakeWidget) == fileListIdx);
        wid = m_mainSplitter->replaceWidget(fileListIdx, m_fileList);
        m_mainSplitterFakeWidget->setParent(this);
        m_mainSplitterFakeWidget->hide();
